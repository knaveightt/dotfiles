...
* Hooks and Wrap Ups
I use one hook to auto-execute commands upon the window manager starting. I am experimenting with another hook to force automatically switching the current group when a window is created and sent to a specified group due to a Match.
